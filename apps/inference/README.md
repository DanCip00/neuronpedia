...
It is for vector readouts, which are the one thing a pod cannot resolve at startup:
a vector is a database row that arrives with the request, so the layer it reads is
...
and a write, so a vector can also be steered at the layer it was fitted at.
...
The current serving contract accepts exactly one input and one SAE source per request and supports only single-GPU vLLM (`tensor_parallel_size=1`). Steered requests are rejected when static CUDA-graph writes or speculative decoding could bypass the schedule. Chunked prefill is fine: the worker hook tracks each request's absolute row position, so the edited row is found in whichever chunk vLLM schedules it in, and a prompt recomputed after preemption is edited again. The route is non-streaming.

Both arms of a comparison take the same path. A steered request cannot share KV with anyone (its cache would be computed from an edited residual), and the baseline opts out of the prefix cache as well, so its prompt KV comes from its own prefill rather than from whichever earlier request happened to populate the cache. On an idle, warmed-up engine that is enough for identical requests to be bitwise repeatable (measured: 5 repeats of baseline and of ablate on three prompts, every logprob identical). Two things break that, and both are properties of the engine rather than of this route. Concurrency: a request's numerics depend on what else is in its batch, so requests that overlap in time differ at the bf16-noise level, and the first minute or so after a restart behaves the same way while kernels are still being selected -- send a few throwaway requests before measuring. And bf16 logits: the lm_head runs in the model dtype, so the difference between two tokens' logprobs is quantized to one bf16 step of the logits, 0.125 for logits in [16, 32). A pair of tokens whose logits are within one step is a coin toss that any upstream bit flip decides either way -- `add` of +2 and of −2 on the same feature both flipped one such pair the same way here -- so do not read a single flipped token as an effect. The first generated token's logprob, which the edited row feeds directly, is the reliable readout: sign-symmetric and growing with the coefficient.

Which SAE is loaded where: `add` never touches an encoder -- the API process pulls the requested `W_dec` rows from its own copy and ships them to the worker with the request. `scale` and `ablate` need the full encoder in the vLLM worker process for TopK selection, so the worker loads it on the first such request and keeps it resident (about 1.6 GB in bf16 for an 80k x 5120 SAE). That first request pays the load, roughly 5 s; every later one costs the same as an `add`.
...
Cost is almost entirely decode steps: the hooked engine runs eagerly, so each generated token is a fixed per-step cost (~75 ms on an idle H200 for Qwen3.5-27B, about double when the GPU is shared with another job) regardless of how many requests share the step, and the per-request steering bookkeeping (two worker RPCs, a few KB of decoder vectors, one SAE encode per edited row) is a few milliseconds. Keep `maxNewTokens` to what the test needs -- one token for a next-token effect -- and get throughput from concurrent requests rather than from anything inside one request: 8 in flight ran at ~6x the throughput of one. Concurrent requests do share batches, though, so for a baseline-versus-steered pair whose logprobs will be subtracted, send the two arms back to back on an otherwise idle engine rather than in the same burst.
