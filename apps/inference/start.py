...
def parse_args():
    ...
    parser.add_argument(
        ...
        "hyper-connection trunk); 'sae' declares the SAE hook sites once those SAEs load; "
        "'sae+auto' declares both, which is what a pod needs when its SAEs read points 'auto' does "
        "not and its lens needs layers the SAEs do not cover; a JSON "
